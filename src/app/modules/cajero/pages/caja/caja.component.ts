...
export class CajaComponent implements OnInit {
  ...
  private mesasSubscription: Subscription | undefined;
  ...
  mesaActual: MesaSeleccionada | undefined;
  ...
  cajaForm: FormGroup = this.FormBuilder.group({
    caja_chica: new FormControl(0, [Validators.required]),
  });
  ...
  changeDescuetoInteresSelected(descuento: number) {
    this.descuentoInteresSelected =
      this.configuracionCaja.descuentosIntereses[descuento];
    this.refreshPecios();
  }
  async guardarCaja() {
    if (this.cajaForm.valid) {
      ...
      this.AlertService.showWarning(
        'No se pudo abrir la caja, ingrese una cantidad.'
      );
    }
  }
  modalGasto() {
  ...
  descuentoAgregado: number = 0;

  modalCierre() {
    if (this.pedidos.length > 0) {
      let respuesta = this.AlertService.showConfirm(
        'Aún se encuentran pedidos por atender, ¿desea cerrar la caja?'
      ).then((res: boolean) => {
        if (res) {
          const dialogRef = this.dialog.open(CerrarCajaComponent, {
            data: {
              caja: this.caja,
              permiso: this.configuracionCaja.cierreCaja,
            },
          });
          dialogRef.afterClosed().subscribe((result) => {
            this.caja = result.caja;
  ...
  ngOnInit(): void {
    this.ConfiguracionService.getConfiguracionCaja().subscribe((res) => {
      ...
      this.descuentoInteresSelected =
        this.configuracionCaja.descuentosIntereses[0];
    });
    ...
    this.mesasSubscription = this.MesaService.mesas.subscribe((mesas) => {
      ...
      if (this.mesaActual != undefined) {
        ...
        if (this.mesaActual?.estado != 0) {
          this.PedidoService.getPedido(this.mesaActual?._id).subscribe(
            (res) => {
              if (res != null) {
                this.pedidoTotal = res as Pedido;
                this.pedidoReal = JSON.parse(JSON.stringify(this.pedidoTotal));
                this.pedidoTotal.pedidos = this.pedidoTotal.pedidos?.filter(
                  (ped) => ped.cantidad_servida != 0
                );
              }
            }
          );
    ...
    this.pedidosSubscription = this.PedidoService.pedidos.subscribe(
      (pedidos) => {
        ...
        const pedidoFind = this.pedidos.find(
          (ped) => ped.id_mesa == this.pedidoTotal.id_mesa
        );
      ...
      if (this.mesaActual.estado >= 2) {
        this.PedidoService.getPedido2(this.mesaActual._id).subscribe((res) => {
          if (res != undefined) {
            ...
            this.pedidoTotalPagar = JSON.parse(
              JSON.stringify(this.pedidoTotal)
            );
            this.pedidoSeleccionado = JSON.parse(
              JSON.stringify(this.pedidoTotal)
            );
          }
          this.drawer.toggle();
  ...
  drop(event: CdkDragDrop<PlatoPedido[]>) {
    ...
    if (event.previousContainer === event.container) {
      ...
      if (
        this.pedidoTotal.pedidos?.find(
          (pTA) => pTA.plato._id == pedido.plato._id
        ) != undefined
      ) {
        if (pedido.cantidad_servida == 1) {
          this.pedidoTotalAux.pedidos = this.pedidoTotalAux.pedidos?.filter(
            (ped) => ped.plato._id != pedido.plato._id
          );
        }
        ...
        if (this.pedidoTotalPagar != undefined) {
          if (this.pedidoTotalPagar.pedidos != undefined) {
            this.pedidoTotalPagar.pedidos.map((ped: any) => {
              if (ped.plato._id == pedido.plato._id) {
                ped.cantidad_servida += 1;
              }
            });
          }
          if (this.pedidoTotalAux != undefined) {
            if (this.pedidoTotalAux.pedidos != undefined) {
              let pedidoss = this.pedidoTotalAux.pedidos.find(
                (ped) => ped.plato._id == pedido.plato._id
              );
              if (pedidoss != undefined) {
                if (pedidoss.cantidad_servida > 1) {
  ...
  dropAux(event: CdkDragDrop<PlatoPedido[]>) {
    ...
    if (event.previousContainer === event.container) {
      ...
      if (
        this.pedidoTotalAux.pedidos?.find(
          (pTA) => pTA.plato._id == pedido.plato._id
        ) != undefined
      ) {
        if (pedido.cantidad_servida == 1) {
        ...
        let pedidoss = this.pedidoTotal.pedidos?.find(
          (ped) => ped.plato._id == pedido.plato._id
        );
        if (pedidoss != undefined) {
          if (pedidoss.cantidad_servida > 1) {
  ...
  cantidadPedido(pedido: PlatoPedido) {
    let encontrado = this.pedidoTotalAux.pedidos?.find(
      (ped) => ped.plato._id == pedido.plato._id
    );
  ...
  movePedidoPago(pedidoPre: PlatoPedido) {
    ...
    if (
      ...
      let pedidosss = this.pedidoTotal.pedidos?.find(
        (ped) => ped.plato._id == pedido.plato._id
      );
      if (pedidosss != undefined) {
        if (pedidosss.cantidad_servida > 1) {
          this.pedidoTotal.pedidos?.map((ped) => {
            if (ped.plato._id == pedido.plato._id) {
              ped.cantidad_servida -= 1;
            }
  ...
  canPay() {
    if (
      this.pedidoTotalAux.pedidos?.length != 0 &&
      this.clienteSelected.nombre_cliente != ''
    ) {
      return false;
    }
    return true;
  ...
  async refreshPecios() {
    ...
    await this.pedidoTotalAux.pedidos?.map((ped) => {
      this.subTotal += Number(
        (ped.plato.precio_plato * ped.cantidad_servida).toFixed(2)
      );
    });
    ...
    if (this.descuentoInteresSelected.descuentoIncremento) {
      this.interes +=
        Math.abs(
          this.subTotal * (this.descuentoInteresSelected.porcentaje / 100)
        ) + this.descuentoInteresSelected.valor;
    } else {
      this.descuento +=
        Math.abs(
          this.subTotal * (this.descuentoInteresSelected.porcentaje / 100)
    ...
    if (this.metodoSelected.descuentoIncremento) {
      this.interes +=
        Math.abs(this.subTotal * (this.metodoSelected.porcentaje / 100)) +
        this.metodoSelected.valor;
    } else {
      this.descuento +=
        Math.abs(this.subTotal * (this.metodoSelected.porcentaje / 100)) +
        this.metodoSelected.valor;
    ...
    this.subTotalConDescunto = this.subTotal - this.descuento;

    if (this.configuracionCaja.checkIVA) {
      this.subTotalConDescunto = Number(
        (this.subTotalConDescunto / 1.12).toFixed(2)
      );
    }

    this.total = Number(Math.abs(this.subTotalConDescunto).toFixed(2));
    this.subTotalIva = Math.abs(
      this.total * (this.configuracionCaja.iva / 100)
    );
  ...
  pagar() {
    ...
    dialogRef.afterClosed().subscribe((data) => {
      if (data.transferencia != undefined) {
        ...
        if (this.caja != null) {
          this.caja.cantidad_descuentos =
            this.caja.cantidad_descuentos + this.descuento;
          this.caja.cantidad_intereses =
            this.caja.cantidad_intereses + this.interes;
          this.caja.cantidad_impuestos =
            this.caja.cantidad_impuestos + this.subTotalIva;
          this.caja.cantidad_ingreso = this.caja.cantidad_ingreso + this.total;
          this.caja.cantidad_efectivo =
            this.caja.cantidad_efectivo + (efectivo - vuelto);
          this.caja.cantidad_transferencia =
            this.caja.cantidad_transferencia + transferencia;
        }

        this.ComprobanteService.guardarComprobante(comprobante).subscribe(
          (res) => {
        ...
        this.pedidoReal.pedidos?.map((pR) => {
          let pedidoFind = this.pedidoTotal.pedidos?.find(
            (p) => p.plato._id == pR.plato._id
          );
              ...
              this.PedidoService.editarPedido(this.pedidoReal).subscribe(
                (res) => {
                  this.AlertService.showSuccess(
                    'Todos los platos fueron pagados.'
                  );
                }
              );
              this.clean();
...
export class DialogPagar {
  ...
  PagarForm: FormGroup = this.formBuilder.group({
    efectivo: new FormControl(0, [Validators.required]),
    transferencia: new FormControl(0, [Validators.required]),
  });
  total: number;
  submitForm() {
    ...
    if (Number(efectivo) >= 0 && Number(transferencia) >= 0) {
      ...
      if (efectivo + transferencia >= this.total) {
        ...
        this.AlertService.showWarning(
          'La cantidad debe ser de ' +
            this.total.toLocaleString('en-US', {
              style: 'currency',
              currency: 'USD',
            }) +
            ' y el total ingresado fue de ' +
            (Number(efectivo) + Number(transferencia)).toLocaleString('en-US', {
              style: 'currency',
              currency: 'USD',
            })
        );
